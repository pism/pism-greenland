...
bed_elevation = np.where(mask, -250, bed_elevation)


A = 500.0
sigma = 1000.0
x0, y0 = -197000.0, -2274500.0

Z = A * np.exp(-((X - x0) ** 2) / (2 * sigma ** 2) - ((Y - y0) ** 2) / (2 * sigma ** 2))
mask = np.zeros((ny, nx))
mask[(X - x0) ** 2 + (Y - y0) ** 2 < A ** 2] = 1
bed_elevation = np.where(mask, Z - 500, bed_elevation)
